...
if __name__ == '__main__':
    ...
    map_num = 1
    gamestate = open("./maps/map" + str(map_num) + ".txt").read()
    window = PlanetWarsWindow(gamestate=gamestate, players=players, max_game_length=500)
    window.paused = False
    app.run()

    with open("./statistics/statistics.txt", "a") as outFile:
        str_out = "Game: 2" + "\tSteps: " + str(window.game.tick) + "\tWinner: " + \
                  str(window.game.winner) + "\tMap: map" + str(map_num)
        for player in window.game.players:
            str_out += "\tPlayer: " + str(player)
        str_out += "\n"
        outFile.write(str_out)
    # for i in range(5):
    #
    #     gamestate = open("./maps/map" + str(map) + ".txt").read()
    #     window = PlanetWarsWindow(gamestate=gamestate, players=players, max_game_length=500)
    #     window.paused = False
    #     app.run()
    #
    #     with open("./statistics/statistics.txt", "a") as outFile:
    #         str_out = "Game: " + str(i + 1) + "\tSteps: " + str(window.game.tick) + "\tWinner: " + \
    #                   str(window.game.winner) + "\tMap: map" + str(map)
    #         for player in window.game.players:
    #             str_out += "\tPlayer: " + str(player)
    #
    #         str_out += "\n"
