...
def choose_action():
    ...
    for key, value in actions.items(): 
        ...
        if best_goal in value:
            # do we have the best key if not, use this one
            if (strBestKey is None and intHighest is None) or actions[key][best_goal] > intHighest:
                strBestKey = key
                intHighest = actions[key][best_goal]
            
            # Do we currently have a "best action" to try? If not, use this one
            if best_action is None:
                if actions[key][best_goal] + goals[best_goal] >= 0:
                    best_action = key
