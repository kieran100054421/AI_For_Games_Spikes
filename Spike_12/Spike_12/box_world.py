...
edge_cost_matrix = [
    # '.'   'm'   '~'  'h'  'X'
    [1.0,  2.0,  5.0, -5.0, None],  # '.'
    [2.0,  4.0,  9.0, -5.0, None],  # 'm'
    [5.0,  9.0, 10.0, -5.0, None],  # '~'
    [1.0,  9.0,  2.0, -5.0, None],  # 'h'
...
box_kind_color = {
    ...
    'h': (0.0, 1.0, 0.0, 1.0),  # health light green
}
...
class BoxWorld(object):
    ...
    def draw(self):
        ...
        if self.path:
            ...
            if self.agent:
                egi.set_pen_color(name=self.agent.colour)
                egi.set_stroke(3)
                self.agent.update_path(self.path.path)
                egi.circle(self.boxes[self.agent.path[self.agent.pos]]._vc, self.agent.radius, True)
